...
async function populateSelects() {
    try {
        ...
        bookSelect.innerHTML = books
            .map((b) => `<option value="${b._id}" ${b.availableCopies < 1 ? "disabled" : ""}>${escapeHtml(b.title)} — ${b.availableCopies} left</option>`)
            .join("");
        memberSelect.innerHTML = members
            .map((m) => `<option value="${m._id}" ${m.status !== "active" ? "disabled" : ""}>${escapeHtml(m.name)}${m.status !== "active" ? " (inactive)" : ""}</option>`)
            .join("");
    }
    catch (err) {
...
tbody.addEventListener("click", async (e) => {
    ...
    if (action === "return") {
        ...
        try {
            ...
            showToast(updated.fineAmount > 0 ? `Returned. Fine due: ${updated.fineAmount}` : "Returned on time — no fine", updated.fineAmount > 0 ? "error" : "success");
            await populateSelects();
            await loadRecords();
        }
        catch (err) {
